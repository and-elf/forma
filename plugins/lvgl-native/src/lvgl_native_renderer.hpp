...
public:
    ...
    bool render_with_io(const void* doc, const char* input_path, const char* output_path, const forma::io::StreamIO& io, forma::HostContext* host = nullptr) {
        ...
        if (input_path) {
            ...
            std::string config_path = (input_file.parent_path() / "forma.toml").string();

            if (auto toml_content = io.open_read(config_path)) {
                try {
                    auto toml_doc = toml::parse<16>(*toml_content);
        ...
        if (!forma::sdl3::download_sdl3(sdl3_version, host)) {
            std::cerr << "Failed to download SDL3\n";
            return false;
        }

        if (!forma::lvgl::download_lvgl(lvgl_version, host)) {
        ...
        // Write output file using StreamIO
        auto parent = std::filesystem::path(output_path).parent_path().string();
        if (!parent.empty()) io.create_dirs(parent);
        if (!io.open_write(output_path, code.str())) return false;

        return true;
    }

    // Backwards-compatible wrapper that uses RealFileSystem-backed StreamIO
    bool render(const void* doc, const char* input_path, const char* output_path) {
        forma::fs::RealFileSystem realfs;
        auto io = forma::io::StreamIO::from_filesystem(realfs);
        return render_with_io(doc, input_path, output_path, io);
    }
...
// Host-aware render entrypoint
extern "C" bool forma_render_host(void* host_ptr, const void* doc, const char* input_path, const char* output_path) {
    auto* host = static_cast<forma::HostContext*>(host_ptr);
    forma::lvgl_native::LVGLNativeRenderer renderer;

    // For host-aware behavior, attempt to read config via host->stream_io (already used inside render_with_io)
    // But download helpers also need the host pointer; we'll call them from here if necessary.
    // The renderer currently calls forma::sdl3::download_sdl3 and forma::lvgl::download_lvgl; those now have host-aware overloads.

    // Call the IO-backed renderer but ensure that downloaders get the host pointer where used.
    // To do that, temporarily call the host-aware download functions directly when needed inside helpers.
    return renderer.render_with_io(doc, input_path, output_path, host->stream_io, host);
}
