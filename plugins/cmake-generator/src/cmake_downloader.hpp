...
public:
    ...
    static bool download_and_install(const std::string& install_dir) {
        return download_and_install(install_dir, nullptr);
#ifdef FORMA_HAS_DOWNLOAD
        ...
        realfs.create_dirs(install_dir);
    ...
    // Host-aware variant
    static bool download_and_install(const std::string& install_dir, forma::HostContext* host) {
#ifdef FORMA_HAS_DOWNLOAD
        std::string url = get_download_url();
        if (url.empty()) {
            std::cerr << "Unsupported platform for CMake download\n";
        ...
        bool success = false;
        if (host) {
            // call host-aware download_and_extract via C API wrapper
            success = ::forma_download_and_extract_host(host, url.c_str(), install_dir.c_str(), 1, nullptr);
        } else {
            success = download::download_and_extract(url, install_dir, 1, opts);
        }
    ...
    // Host-aware variant
    static std::string ensure_cmake_available(const std::string& project_root, forma::HostContext* host) {
        // First check if cmake is on PATH
        if (is_cmake_available()) {
            return "cmake"; // Use system cmake
        }
        ...
        // Need to download
        if (!download_and_install(cmake_dir, host)) {
            return ""; // Download failed
        }
